...
import {AttributeTypes, AttributeResolvers, AttributePath} from '../attributes';

declareElementFactory('component', (reporter: Reporter, name: string, definition: MakeJS.Element, attrPath: AttributePath, parent: Element) => {
...
export class ComponentElement extends Element {
    ...
    for (let depcomponent of elements) {
      ...
      for (let key of keys) { if (ComponentElement.notInjectedKeys.has(key)) continue;
        ...
        if (cvalue !== undefined && cvalueIsArr !== dvalueIsArr) {
          reporter.diagnostic({
            type: "warning",
            msg: `attribute '${key}' value is incoherent for component ${this.__path()} while injecting ${depcomponent.__path()}, attribute is ignored`
          });
        }
        else if (dvalueIsArr) {
          if (!cvalue)
          ...
          reporter.diagnostic({
            type: "warning",
            msg: `attribute '${key}' value is incoherent for component ${this.__path()} while injecting ${depcomponent.__path()}, attribute is removed`
          });
          delete this[key];
        }
  ...
  __resolve(reporter: Reporter) {
    ...
    this.components = this.__resolveElements(reporter, this.components, 'components', 'component');
